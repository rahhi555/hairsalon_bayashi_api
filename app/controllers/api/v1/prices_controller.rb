...
module Api
  module V1
    class PricesController < ApplicationController
      before_action :set_price, only: %i[show update destroy order_stylists]
      ...
      def base
        ...
        lowest_rank = Rank.order(name: :desc).pick(:id)
      ...
      def order_stylists
        ...
        new_apo_on = Time.zone.parse(params[:appointmentOn])

        # 予約希望時間と現在の予約を比較して、対応不可能なスタイリストのidを返す(最後にcompactを入れないとnilが入ってしまう)。
        free_stylists = already_apo_times.map do |apo|
